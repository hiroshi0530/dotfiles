# 基本
- 作業は必ず `git worktree` で別の作業ディレクトリを作成してから実行すること
- worktree のディレクトリ名は編集する目的を表す名前にすること（例: `../worktree-add-zsh-alias`、`../worktree-fix-path-config`）
- 日本語で応答すること
- 絵文字を使用しないこと（コード・コミットメッセージ・ドキュメント・SKILL.md を含むすべての出力で禁止）
